<script>
	...
	async function submitVoucher() {
		try {
			const hash = voucherLink.replace('https://gift.truemoney.com/campaign/?v=', ''); // Extract hash
			const userID = parseInt(localStorage.getItem('userID') || ''); // Get userID from localStorage

			// Make POST request to the initiate API
			const response = await fetch('http://localhost:3000/users/manybaht/initiate', {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json'
				},
				body: JSON.stringify({ hash, userID }) // Send hash and userID in the body
			});

			const data = await response.json();

			if (response.ok) {
				// Update balance on successful top-up
				balance += parseFloat(data.amount); // Assuming 'data.amount' is the top-up amount

				localStorage.setItem('balance', balance.toString()); // Update balance in localStorage

				alert('Top-up successful! Your new balance is: ' + balance);
			} else {

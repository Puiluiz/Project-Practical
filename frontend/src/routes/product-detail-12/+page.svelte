	...
	async function buyProduct(productID) {
		...
		try {
			const response = await fetch(
				`http://localhost:3000/products/12months/buy/${userID}/${productID}`,
				{
					method: 'POST',
					headers: {
		...
		<div class="flex flex-col md:flex-row items-center justify-center py-10 md:py-20">
			...
			{#if products12Months.length == 0}
				<div>
					...
					>
						<h1 class="font-mitr font-regular text-4xl text-[#2C2C2C] mb-4">12 MONTH</h1>
						...
						<h1 class="font-mitr font-regular text-3xl text-[#2C2C2C]">IN STOCK {products12Months.length}</h1>
					</button>
				</div>
